...
def main():
    ...
    # train Rachel
    X_ph, y_ph = diag.rnnPrep(100, character=1)
    model_ph = diag.rnnTrain(X_ph, y_ph)
    print('Done')
    diag.writeOutput(model_ph)
    
    # train Ross
    X_ph, y_ph = diag.rnnPrep(100, character=2)
    model_ph = diag.rnnTrain(X_ph, y_ph)
    print('Done')
    diag.writeOutput(model_ph)
    
    # train Monica
    X_ph, y_ph = diag.rnnPrep(100, character=3)
    model_ph = diag.rnnTrain(X_ph, y_ph)
    print('Done')
    diag.writeOutput(model_ph)
    
    # train Chandler
    X_ph, y_ph = diag.rnnPrep(100, character=4)
    model_ph = diag.rnnTrain(X_ph, y_ph)
    print('Done')
    diag.writeOutput(model_ph)
    
    # train Joey
    X_ph, y_ph = diag.rnnPrep(100, character=5)
    model_ph = diag.rnnTrain(X_ph, y_ph)
    print('Done')
    diag.writeOutput(model_ph)
